...
use aries_vcx::init::{open_main_pool, PoolConfig, open_as_main_wallet, init_issuer_config};
        ...
        None => match std::env::var("LEDGER_URL").ok() {
            Some(ledger_url) => {
                ...
                let body = reqwest::get(&genesis_url)
                    ...
                    .text()
                    .await.unwrap();
                let path = std::env::current_dir().expect("Failed to obtain the current directory path").join("genesis_file.txn");
            ...
            None => {
                std::env::current_dir().expect("Failed to obtain the current directory path").join("indypool.txn").to_str().map(|s| s.to_string()).ok_or("Failed to convert genesis file path to string".to_string())
            }
        }
    }
}
...
pub async fn initialize() -> std::io::Result<AgentConfig> {
    ...
    info!("prepare pool config");
    let pool_config = PoolConfig {
        genesis_path,
        pool_config: None,
        pool_name: None
    };
    info!("prepare agency config");
